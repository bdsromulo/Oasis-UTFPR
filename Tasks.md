...
- **BUG-04 — No celular, a barra de grade e o convite de Novidades ficavam ativos na própria tela de check-in, antes de curso ou histórico:**
  - **Reproduzido (2026-08-14) em viewport 375×812, sessão limpa:** com `localStorage`/`sessionStorage` vazios — visitante nunca antes na plataforma —, a tela de check-in no celular já trazia, abaixo do rodapé, a barra flutuante `Grade A · 0 turmas · Abrir Grade`. Tocar nela abre a gaveta **Mini-Grade no Celular** completa: grade semanal M1–N5, filtro de turno, e o botão `+` para criar `Grade B`/`C`. Testei o `+` e ele **de fato criou uma Grade B funcional**, com botão de remover — não é vazamento só visual, é o mesmo estado (`cestaGrades`) que a tela de Planejamento usa depois do check-in, mutado antes de o aluno escolher curso.
  - **Segundo vazamento, mesma causa raiz:** o ícone `★ Novidades` do cabeçalho mobile também aparecia na tela de check-in e abria o modal completo de avaliações da comunidade — citando telas e botões (`Planejamento`, `Configurações`) que quem ainda não entrou na plataforma nunca viu.
  - **Causa:** dois blocos de UI exclusivos do celular (`sm:hidden`/`lg:hidden` em `src/ui/App.tsx`) checavam apenas o estado de **aba/tela** (`aba === "planejamento"`, `sobreAberta`, etc.) ou de **disponibilidade de feature** (`reviewsHabilitadasPara`), nunca o gate de **sessão** (`perfil`/`checkinConcluido`). O bloco irmão do desktop (linha ~752) já envolvia as mesmas ações em `{(perfil || checkinConcluido) && (...)}` — a assimetria mobile/desktop é o defeito. Como `aba` nasce como `"planejamento"` quando não há perfil salvo (`useState(() => (lerPerfil() ? "situacao" : "planejamento"))`), a condição da barra de grade dava verdadeiro exatamente na primeira visita.
  - **Por que o dado exibido era enganoso além de prematuro:** sem curso escolhido, a barra usa o curso padrão (`cursoAtivo ?? "bsi-981"`) em silêncio — um visitante pretendendo Eng. Elétrica veria/mexeria numa grade de BSI sem indicação nenhuma disso.
  - **Confirmado que é o caminho de celular real, não só tela estreita:** `sm:` e `lg:` são os breakpoints padrão do Tailwind v4 (640px/1024px) — o projeto não sobrescreve nenhum dos dois nem faz detecção de dispositivo em lugar nenhum. Qualquer telefone real, retrato ou paisagem, fica bem abaixo de 640px. O teste usou o perfil `mobile` da ferramenta de preview, que emula Android Chrome de verdade (user-agent, 5 pontos de toque, tradução de mouse para toque) e não apenas redimensiona a janela — mesmo mecanismo de breakpoint que o resto do "mobile" do projeto já usa (`MenuMobile`, barra inferior de `SidebarNavegacao`, validação de viewport da TASK-36).
  - **Correção:** `barraGradeMobileVisivel` e o botão `Novidades` do cabeçalho mobile passaram a exigir `(!!perfil || checkinConcluido)`, replicando a condição já usada no bloco desktop. Nenhum código novo, só a mesma guarda de sessão aplicada nos dois lugares que faltava.
  - **Validado:** suíte completa 36/36 arquivos, 436 aprovados, 0 falhas; `tsc --noEmit` limpo; build de produção sem mudança de tamanho (407,6 KiB gzip). Reproduzido no navegador em 375×812 com storage limpo — sem `Abrir Grade` nem `Novidades` na tela de check-in — e confirmado que o fluxo pós-check-in (Modo Livre) segue mostrando os dois normalmente. Viewport desktop (1280×720), que já estava correto, sem mudança.
  - Publicado em `main` (merge de `fix/mobile-gating-pre-checkin`).
