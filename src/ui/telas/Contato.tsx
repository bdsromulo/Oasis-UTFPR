 ...
 */
export function PilulaFaleConosco() {
  const [aberta, setAberta] = useState(false);
  const { copiado, copiar } = useCopiarEmail();

  return (
    <div className="fixed bottom-24 right-4 z-30 flex flex-col items-end gap-2 lg:bottom-4">
      {aberta && (
        <div className="animate-in fade-in zoom-in-95 w-60 rounded-2xl border border-zinc-200/90 bg-white p-3.5 shadow-2xl duration-150 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <a
            href={LINK_MAILTO}
            className="mt-2 block break-all rounded-lg bg-zinc-100 px-2 py-1.5 font-mono text-[11px] font-bold text-zinc-800 transition-colors hover:bg-utfpr-500/20 dark:bg-zinc-800 dark:text-zinc-100"
          >
            {EMAIL_CONTATO}
          </a>
          <button
            type="button"
            onClick={copiar}
            className="mt-2 flex w-full cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-zinc-200 px-2 py-1.5 font-display text-[11px] font-bold text-zinc-600 transition-colors hover:border-utfpr-500 hover:text-zinc-900 dark:border-zinc-700 dark:text-zinc-300 dark:hover:text-white"
          >
            {copiado ? (
              <>
  ...
  if (props.compacto) {
    return (
      <a
        href={LINK_MAILTO}
        title={`Fale conosco — ${EMAIL_CONTATO}`}
        className="flex shrink-0 items-center gap-1.5 rounded-xl border border-utfpr-500/50 bg-utfpr-500/10 px-2.5 py-1.5 font-display text-xs font-bold text-amber-900 transition-colors hover:bg-utfpr-500/25 dark:text-utfpr-300"
      >
