...
export function Badge(props: {
  ...
  const tons = {
    ok: "border border-emerald-200/80 bg-emerald-50/90 text-emerald-800 dark:border-emerald-800/60 dark:bg-emerald-950/60 dark:text-emerald-300",
    alerta: "border border-red-200/80 bg-red-50/90 text-red-800 dark:border-red-800/60 dark:bg-red-950/60 dark:text-red-300",
    neutro: "border border-zinc-200/80 bg-zinc-100/90 text-zinc-700 dark:border-zinc-800/60 dark:bg-zinc-800/80 dark:text-zinc-300",
    acento: "border border-utfpr-500/60 bg-gradient-to-r from-utfpr-500/20 to-amber-500/15 text-amber-900 font-bold dark:border-utfpr-500/30 dark:bg-utfpr-500/15 dark:text-utfpr-400",
  };
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-semibold transition-colors ${tons[props.tom ?? "neutro"]} ${props.classe ?? ""}`}
    >
      {props.icon}
      {props.children}
    </span>
